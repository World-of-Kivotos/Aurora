...
use aurora_modpack::snapshot::SnapshotWorkingDirectory;

use common::{SHA_A, SHA_B, WORKING_DIRECTORY, disk, manifest, previous, remote, snapshot};
...
fn row_remote_only_missing_downloads_new_file() {
    ...
    let plan = diff(&remote_manifest, None, &[], WORKING_DIRECTORY).unwrap();
...
fn row_remote_and_snapshot_present_but_disk_missing_redownloads_every_policy() {
    for policy in [
        ...
        let plan = diff(&remote_manifest, Some(&old), &[], WORKING_DIRECTORY).unwrap();
...
#[test]
fn snapshot_from_another_working_directory_is_rejected_before_deletion_planning() {
    let remote_manifest = manifest(vec![]);
    let old = snapshot(vec![previous(PATH, FilePolicy::Managed, SHA_A)]);
    ...
    assert!(matches!(
        ...
        Err(Error::SnapshotWorkingDirectoryMismatch {
            snapshot: SnapshotWorkingDirectory::IsolatedVersionDirectory,
            current: SnapshotWorkingDirectory::SharedGameDirectory,
        })
    ));
}
...
fn case_aliases_in_disk_inventory_are_rejected() {
    ...
    assert!(matches!(
        diff(&remote_manifest, None, &disk_files, WORKING_DIRECTORY),

...
impl Aurora {
    ...
    pub(crate) async fn install_vanilla_component(
        &self,
        id: &str,
        events: Option<&EventSink>,
    ) -> Result<VanillaSummary> {
    ...
    pub(crate) async fn install_loader_component(
        &self,
        id: &str,
        loader: Option<LoaderChoice>,
        loader_version: Option<&str>,
        events: Option<&EventSink>,
    ) -> Result<Option<LoaderSummary>> {
        let Some(choice) = loader else {
            return Ok(None);
        };
        let layout = self.layout();
        let pool = self.download_pool();
    ...
    pub(crate) async fn install_loader_component_from_installer(
        &self,
        id: &str,
        choice: LoaderChoice,
        loader_version: &str,
        ...
        if !matches!(choice, LoaderChoice::Forge | LoaderChoice::NeoForge) {
            return Err(CoreError::ModpackMetadataConflict {
                detail: format!(
                    "prepared installer file cannot be used for {}",
                    choice.display_name()
                ),
            });
        }
        ...
        let summary = self
            .install_forge(
                ...
                choice,
                loader_version,
                ForgeInstallerSource::PreparedFile(installer),
                context,
                events,
            )
            .await?;
    ...
    /// [`Aurora::prepare_java`] 取得可执行 java。安装源可以是普通安装流程给出的官方 URL，也可以是受管
    /// 整合包 preflight 已检查过的缓存文件；本方法只负责 Java 解析、执行与把 [`aurora_install::ForgeSummary`]
    ...
    async fn install_forge(
        ...
        let installer = ForgeInstaller::new(cx, java_path);
        let summary = match source {
            ForgeInstallerSource::Url(url) => {
                ...
                installer.install(url).await?
            }
            ForgeInstallerSource::PreparedFile(path) => {
                emit(
                    events,
                    CoreEvent::stage(format!(
                        "执行已预检的 {} 安装器：{}",
                ...
                installer.install_from_installer(path).await?
            }
        };
        emit(
            events,
...
mod tests {
    ...
    async fn mount_managed_vanilla_fixture(
        server: &MockServer,
        base: &str,
        client_bytes: &[u8],
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(
                r#"{
                    ...
                    "version":"2.0.0",
                    "minecraft":"test",
                    "loader":{"kind":"vanilla","version":"vanilla"},
                    "files":[]
                }"#,
            ))
            .mount(server)
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(format!(
                ...
                sha1_hex(client_bytes),
                client_bytes.len()
            )))
            .mount(server)
            .await;
        let client_response = if client_available {
            ResponseTemplate::new(200).set_body_bytes(client_bytes.to_vec())
        } else {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn managed_vanilla_install_ignores_unrelated_instances_and_finishes_snapshot() {
        let server = MockServer::start().await;
        let base = server.uri();
        Mock::given(method("GET"))
            .and(path("/latest"))
        ...
        let unrelated = mc.join("versions").join("unrelated");
        tokio::fs::create_dir_all(&unrelated).await.unwrap();
        tokio::fs::write(
            unrelated.join("unrelated.json"),
            r#"{"id":"unrelated","type":"release","mainClass":"test.Main"}"#,
        )
        .await
        .unwrap();
        ...
        assert_eq!(outcome.instance_id, "test");
        assert_eq!(outcome.installed_version, "2.0.0");
        assert!(unrelated.join("unrelated.json").is_file());
        let subscription = aurora.modpack_subscription("test").await.unwrap().unwrap();
        assert_eq!(subscription.pack_id, "wok");
        assert_eq!(subscription.pointer_url, format!("{base}/latest"));
        let snapshot =
            aurora_modpack::SnapshotStore::for_version_dir(&mc.join("versions").join("test"))
                .load()
                .await
                .unwrap()
                .unwrap();
        ...
        assert_eq!(
            aurora.version_settings("test").await.unwrap().isolation,
            aurora_instance::IsolationOverride::Enabled
        );
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn managed_vanilla_install_resumes_after_client_failure_and_cleans_reservation() {
        let server = MockServer::start().await;
        let base = server.uri();
        let client_bytes = b"managed-client-jar";
        mount_managed_vanilla_fixture(&server, &base, client_bytes, false).await;
        ...
        .with_manifest_url(format!("{base}/manifest.json"));
        let pointer_url = format!("{base}/latest");

        let first_error = aurora
            .install_managed_modpack(&pointer_url, None)
            .await
            .unwrap_err();
        assert_eq!(
            first_error.stage,
            crate::modpack::ModpackSyncStage::InstallingMinecraft
        );

        let version_dir = mc.join("versions").join("test");
        let version_json = version_dir.join("test.json");
        let client_jar = version_dir.join("test.jar");
        let reservation = mc
            .join(".aurora")
            .join("modpack-install-reservations")
            .join("test.json");
        let owner = version_dir.join(".aurora").join("install-owner.json");
        assert!(version_json.is_file(), "失败前应已写入原版 version JSON");
        assert!(!client_jar.exists(), "失败的 client.jar 不应伪装成完整文件");
        assert!(reservation.is_file(), "失败后必须保留持久 reservation");
        assert!(owner.is_file(), "失败后必须保留版本目录 owner");
        assert!(aurora.modpack_subscription("test").await.unwrap().is_none());
        assert!(
            aurora_modpack::SnapshotStore::for_version_dir(&version_dir)
                .load()
                .await
                .unwrap()
                .is_none(),
        ...
        let reservation_json: serde_json::Value =
            serde_json::from_slice(&tokio::fs::read(&reservation).await.unwrap()).unwrap();
        let owner_json: serde_json::Value =
            serde_json::from_slice(&tokio::fs::read(&owner).await.unwrap()).unwrap();
        assert_eq!(reservation_json["instance_id"], "test");
        assert_eq!(owner_json["directory_id"], "test");
        assert_eq!(owner_json["operation_id"], reservation_json["operation_id"]);

        server.reset().await;
        mount_managed_vanilla_fixture(&server, &base, client_bytes, true).await;
        let outcome = aurora
            .install_managed_modpack(&pointer_url, None)
            .await
            .unwrap();
        ...
        let subscription = aurora.modpack_subscription("test").await.unwrap().unwrap();
        assert_eq!(subscription.pack_id, "wok");
        assert_eq!(subscription.pointer_url, pointer_url);
        let snapshot = aurora_modpack::SnapshotStore::for_version_dir(&version_dir)
            .load()
            .await
            .unwrap()
            .unwrap();
        assert_eq!(snapshot.pack_id, "wok");
        assert_eq!(snapshot.version, "2.0.0");
        assert!(snapshot.files.is_empty());
        assert!(!reservation.exists(), "成功后应删除 reservation");
        assert!(!owner.exists(), "成功后应删除版本目录 owner");
    }
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_string(r#"{"latest":{"release":"a","snapshot":"a"},"versions":[]}"#),
            )
            .mount(&server)
            .await;

...
export function Skeleton({ className = "", delay = 0 }: SkeletonProps) {
  return (
    <div data-skeleton className={`relative overflow-hidden rounded-control bg-ink/[0.055] ${className}`}>
      <motion.div
...
export function ResourceCardSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    <div className="flex items-start gap-3.5 rounded-panel border border-ink/8 bg-paper-sink/60 p-3.5">
...
export function VersionCardSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    <div className="rounded-panel border border-ink/8 bg-paper-sink/60 px-3.5 py-3">
